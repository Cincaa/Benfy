...
# with open(DATA_PATH.joinpath('Covid_Romania.json') as json_file:
#     covid_ro_data = json.load(json_file)['covid_romania']
...
def covid_ro_fig(covid_ro_data, base, theme):
    global df1
    df1 = pd.DataFrame(
        dict(
    ...
    fig = custom_legend(
        fig=fig,
        nameSwap={"new_cases_fd": "Cazuri noi", "total_cases_fd": "Cazuri totale"},
    )
...
def number_render_cov(rangeval, theme):
    if rangeval is None:
        return covid_ro_fig(covid_ro_data, 10, theme)
    else:
        return covid_ro_fig(covid_ro_data, rangeval, theme)
...
layout = html.Div(
    [
        html.H1(
            "Legea lui Benford pentru cazurile COVID din România", style=cst.TEXT_STYLE
        ),
        html.Hr(),
                    ...
                    children=[
                        ...
                        html.P(
                            ...
                            children=[
                                ...
                                " se observă o oarecare neconcordanță"
...
def goodness_of_fit(values):
    ...
    if values is None:
        ...
        names = ["cazuri noi", "cazuri totale"]
